...
public class TintedCableBuilder extends CableBuilder {
  ...
  private final EnumMap<CableCoreType, TextureAtlasSprite> tintedCores = new EnumMap<>(CableCoreType.class);
  private final EnumMap<AECableType, TextureAtlasSprite> tintedConnections = new EnumMap<>(AECableType.class);

  public TintedCableBuilder(Function<Material, TextureAtlasSprite> bakedTextureGetter) {
    ...
    for (CableCoreType type : CableCoreType.values()) {
      ...
      this.tintedCores.put(type, bakedTextureGetter.apply(new Material(TextureAtlas.LOCATION_BLOCKS, MAE2.toKey("part/cable/core/"+type.name().toLowerCase()))));
    }

    for (AECableType type : AECableType.VALIDCABLES) {
      this.tintedConnections.put(type, bakedTextureGetter.apply(new Material(TextureAtlas.LOCATION_BLOCKS, MAE2.toKey("part/cable/connection/"+type.name().toLowerCase()))));
    }
  }
  ...
  public void addCableCore(CableCoreType coreType, AEColor color, List<BakedQuad> quadsOut) {
    ...
    TextureAtlasSprite texture = this.tintedCores.get(coreType);
  ...
  public void addGlassConnection(Direction facing, AEColor cableColor, AECableType connectionType,
    ...
    if (connectionType != AECableType.GLASS && !cableBusAdjacent) {
      TextureAtlasSprite texture = this.tintedConnections.get(AECableType.COVERED);
      cubeBuilder.setTexture(texture);

      this.addBigCoveredCableSizedCube(facing, cubeBuilder);
    }

    TextureAtlasSprite texture = this.tintedConnections.get(AECableType.GLASS);
    cubeBuilder.setTexture(texture);
  ...
  public void addSmartConnection(Direction facing, AEColor cableColor, AECableType connectionType,
    ...
    TextureAtlasSprite texture = this.tintedConnections.get(AECableType.SMART);
    cubeBuilder.setTexture(texture);
  ...
  public void addDenseCoveredConnection(Direction facing, AEColor cableColor, AECableType connectionType,
    ...
    TextureAtlasSprite texture = this.tintedConnections.get(AECableType.DENSE_COVERED);
    cubeBuilder.setTexture(texture);
  ...
  public void addDenseSmartConnection(Direction facing, AEColor cableColor, AECableType connectionType,
    ...
    TextureAtlasSprite texture = this.tintedConnections.get(AECableType.DENSE_SMART);
    cubeBuilder.setTexture(texture);
  ...
  public TextureAtlasSprite getCoreTexture(CableCoreType coreType, AEColor color) {
    return this.tintedCores.get(coreType);
  }
}
